...
describe('6.9. WINDOW_UPDATE', function () {
  ...
  describe('6.9.1. The Flow Control Window', function () {
    it('Sends a WINDOW_UPDATE frame with an exceeded maximum window increment', function (done) {
    ...
    it('Sends a WINDOW_UPDATE frame with an exceeded maximum window increment on a stream', function (done) {
      var message = 'the endpoint MUST respond with a connection error of type FLOW_CONTROL_ERROR.';

      var conn = helper.createConnection(function () {
        conn.on('frame', function (frame) {
          if (frame.type === protocol.FRAME_TYPE_RST_STREAM) {
            assert.equal(frame.errorCode, protocol.CODE_FLOW_CONTROL_ERROR, message);
            done();
          }
        });
        ...
        var windowUpdateframe = new Buffer([
          0x00, 0x00, 0x04, 0x08, 0x00, 0x00, 0x00, 0x00,
          0x00, 0x00, 0x80, 0x00, 0x00, 0x00
        ]);
        conn.socket.write(windowUpdateframe);
      });
    });
  });
